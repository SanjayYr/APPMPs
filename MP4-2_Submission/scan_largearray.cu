...
//#define DEFAULT_NUM_ELEMENTS 32768
//1048576 
...
//                h_data[i] = 1.0f;
    ...
    int numBlocks = padded_num_elements/TILE_SIZE;
    unsigned int* sums = NULL;
    cudaMalloc( (void**) &sums, numBlocks);
    
    int padded_num_elementsInc = TILE_SIZE*((numBlocks+TILE_SIZE-1)/TILE_SIZE);
    int padded_mem_sizeInc = padded_num_elementsInc *sizeof(unsigned int);
    unsigned int* incr = NULL;
    cudaMalloc( (void**) &incr, (padded_num_elementsInc)*sizeof(unsigned int));
    cudaMemset(sums+numBlocks, 0, padded_mem_sizeInc-(numBlocks*sizeof(unsigned int)));  
 
    // **===-----------------------------------------------------------===**

    // Run just once to remove startup overhead for more accurate performance 
    // measurement
    prescanArray(d_odata, d_idata, sums, incr, TILE_SIZE, padded_num_elementsInc);
    ...
    prescanArray(d_odata, d_idata, sums, incr, padded_num_elements, padded_num_elementsInc);
    // **===-----------------------------------------------------------===**
