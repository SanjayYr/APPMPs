...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
    ...
    for(int i=0; i < dev_count; ++i)
    {
       ...
       printf("dev_prop.sharedMemPerBlock: %zu\n", dev_prop.sharedMemPerBlock);
       printf("dev_prop.warpSize: %d\n\n", dev_prop.warpSize);
       printf("dev_prop.clockRate: %d\n\n", dev_prop.clockRate);
       printf("dev_prop.multiProcessorCount: %d\n\n", dev_prop.multiProcessorCount);

       printf("dev_prop.regsPerBlock: %d\n", dev_prop.regsPerBlock);
       printf("dev_prop.totalConstMem: %zu\n", dev_prop.totalConstMem);
       printf("dev_prop.totalGlobalMem: %zu\n\n", dev_prop.totalGlobalMem);

       printf("dev_prop.maxThreadsDim[0] (x): %d\n",  dev_prop.maxThreadsDim[0]);
       printf("dev_prop.maxThreadsDim[1] (y): %d\n",  dev_prop.maxThreadsDim[1]);
       printf("dev_prop.maxThreadsDim[2] (z): %d\n\n",  dev_prop.maxThreadsDim[2]);

       printf("dev_prop.maxGridSize[0] (x): %d\n",  dev_prop.maxGridSize[0]);
       printf("dev_prop.maxGridSize[1] (y): %d\n",  dev_prop.maxGridSize[1]);
       printf("dev_prop.maxGridSize[2] (z): %d\n\n", dev_prop.maxGridSize[2]);
    }
    */

    // Setup the execution configuration
    int numBlocksX = N.width/TILE_WIDTH;
    if(N.width%TILE_WIDTH) numBlocksX++;

    int numBlocksY = M.height/TILE_WIDTH;
    if(M.height%TILE_WIDTH) numBlocksY++;
