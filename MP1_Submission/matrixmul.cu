...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
    ...
    int numBlocks = MATRIX_SIZE/MATRIX_SIZE;  // Here both width and BLOCK_WIDTH are equal to MATRIX_SIZE
    if (MATRIX_SIZE % MATRIX_SIZE) numBlocks++; 
    
    dim3 dimGrid(numBlocks, 1, 1);              // numBlocks will be 1 here
    dim3 dimBlock(MATRIX_SIZE, MATRIX_SIZE, 1);  // Since BLOCK_WIDTH=MATRIX_SIZE
    ...
    // Free all cuda memory allocated
